...
class QuizScreen extends StatefulWidget{
  ...
  const QuizScreen({Key? key, required this.practicesId});
...
class _QuizScreen extends State<QuizScreen>{
  ...
  Future<List<QuestionItem>> getQuestions() async {
    ...
    List<QuestionItem> questions = await _apiClient.getPretestQuestions(token!, widget.practicesId);
    return questions;
  }
  ...
  Future<void> saveAnsweredQuestions(String answerOption, String answerNumber) async {
    var token = await storage.read(key: 'token');
    dynamic res = await _apiClient.answerPretest(token!, widget.practicesId, answerNumber, answerOption);
    if (res['success'] == true) {
  ...
  Future<MyPreTestResults> finishPretest() async {
    var token = await storage.read(key: 'token');
    MyPreTestResults result = await _apiClient.finishPretest(token!, widget.practicesId);
