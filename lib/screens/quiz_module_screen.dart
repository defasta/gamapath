...
import 'package:gamapath/core/repository.dart';
...
class QuizModuleScreen extends StatefulWidget{
  ...
  const QuizModuleScreen({Key? key, required this.practicesId});

  @override
  State<QuizModuleScreen> createState() => _QuizScreen();
}

class _QuizScreen extends State<QuizModuleScreen>{
  ...
  Future<List<QuestionItem>> getQuestions() async {
    ...
    List<QuestionItem> questions = await _apiClient.getQuizModuleQuestions(token!, widget.practicesId);
    return questions;
  }
  ...
  Future<void> saveAnsweredQuestions(String answerOption, String answerNumber) async {
    var token = await storage.read(key: 'token');
    dynamic res = await _apiClient.answerQuizModule(token!, widget.practicesId, answerNumber, answerOption);
    if (res['success'] == true) {
  ...
  Future<MyPreTestResults> finishPretest() async {
    var token = await storage.read(key: 'token');
    MyPreTestResults result = await _apiClient.finishQuizModule(token!, widget.practicesId);
                            ...
                            if(_isPreTestFinished == true){
                              ...
                              return Container(
                                child: Padding(
                                    ...
                                    ) :Submit(_backToPreviousQuestion, finishPretest)
